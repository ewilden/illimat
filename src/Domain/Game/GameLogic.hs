...
computeOtherPlayerView (PlayerState hand numOkuses lums harvestPile) =
    OtherPlayerView { _opNumCardsInHand  = length hand
...
computeBoardView (BoardState n e s w) = BoardStateView (computeFieldView n)
                                                       (computeFieldView e)
                                                       (computeFieldView s)
                                                       (computeFieldView w)
...
computeFieldView (FieldState cards lum) = FieldStateView
    cards
    (case lum of
...
dealLuminaryToField dir = do
    ...
    case unusedLums of
        ...
        (h : tl) -> do
            ...
            updateState $ mapFieldM
                dir
                (\fs -> return $ fs { _fieldLuminary = FaceDown h })
...
addCardStacksToHarvestPile playerIndex stacks = do
    ...
    updateState $ mapPlayerM
        ...
        (\prevPlayer -> return $ prevPlayer
            { _playerHarvestPile = (  cardsInStacks
                                   ++ _playerHarvestPile prevPlayer
                                   )
            }
        )
...
doInitialGameSetup = do
    ...
    sequence_
        ( map (\(_, i) -> fillPlayersHandToN 4 i)
        $ filter (\(_, i) -> i /= firstTurnTaker)
        $ zip currPlayers [0 ..]
        )
...
updateWhoseTurn wasRakeSow = do
    ...
    case (rakeIsOnBoard, rakeTurn) of
        ...
        (True, RakeOtherMove) -> updateState
            (\gs -> return $ gs
...
stockpile playerIndex card dir targetStacks desiredStackVal = do
    ...
    let stackIsValid = canHarvestWithNum desiredStackVal
                                         ((fromCard card) : targetStacks)
    checkS stackIsValid
...
sow playerIndex card dir = do
    fieldHasFaceUpRake <-
        withS $ (== FaceUp Rake) . _fieldLuminary . fieldFromDir dir
    currPlayer <- getPlayerS playerIndex
    checkS (card `elem` _playerHand currPlayer)
...
setSeason Summer dir = do
    ...
    let forestQueenIsActive = (FaceUp Forest_Queen) `elem` luminaries
        luminaries          = map _fieldLuminary fields
        fields =
            map (\dir' -> fieldFromDir dir' currGS) (allEnum :: [Direction])
    ...
    if forestQueenIsActive && not willBeSettingSummerForForestQueen
        then returnLeft "Can't change seasons; Forest Queen is active."
            ...
            $ mapIllimatM (\is -> return is { _illSummerDir = dir })
        ...
        case lum of
                ...
                case (_fieldLuminary currOppField) of
                    FaceDown oppLum -> do
                        updateState $ mapFieldM
                            oppDir
                            (\fs ->
                                return $ fs { _fieldLuminary = FaceUp oppLum }
                            )
            ...
            Children -> do
                ...
                case (_gameDeck currGS) of
                    a : b : c : rest -> do
                        put $ currGS { _gameDeck          = rest
                                     , _gameChildrenCards = [a, b, c]
                                     }
                        defaultBehavior
                    _ -> returnLeft
                        "Not enough cards to reserve for the Children!"

resolveLuminaryTake :: Luminary -> PlayerIndex -> FailableGameAction ()
resolveLuminaryTake lum playerIndex = case lum of
    Rake -> do
        let giveCardFromAToB card aInd bInd = do
                updateState $ mapPlayerM
                    aInd
                    (\ps -> return $ ps
                        { _playerHarvestPile = (_playerHarvestPile ps)
                                                   `setDelete` card
                        }
                    )
                updateState $ mapPlayerM
                    bInd
                    (\ps -> return $ ps
                        { _playerHarvestPile = card : (_playerHarvestPile ps)
                        }
                    )
    ...
    Children -> do
        ...
        updateState $ mapPlayerM
            playerIndex
            (\ps -> return $ ps
                { _playerHarvestPile = currChildrenCards
                                           ++ (_playerHarvestPile ps)
                }
            )
    _ -> return ()
...
isGameOver gameState =
    all (== []) $ map _playerHand $ _gamePlayerState gameState

-- scoring
score :: [PlayerState] -> [Int]
score players = flip execState (map (const 0) players) $ do
    let givePointsTo :: Int -> PlayerIndex -> State [Int] ()
        givePointsTo numPoints playerIndex = do
            ...
            put
                ...
                & zip [0 ..]
                & map
                      (\x@(ind, pts) -> if ind == playerIndex
                          then (ind, pts + numPoints)
                          else x
                      )
                & map snd
        indicesOfHighest :: [(PlayerIndex, Int)] -> [PlayerIndex]
        indicesOfHighest scores =
            scores & groupSortBy cmp (:) & headOr [] & map fst
            where cmp (ind1, score1) (ind2, score2) = compare score2 score1
        luminaryCounts :: [(PlayerIndex, Int)]
        luminaryCounts =
            players & map (length . _playerLuminaries) & zip [0 ..]
        cardCounts :: [(PlayerIndex, Int)]
        cardCounts = players & map (length . _playerHarvestPile) & zip [0 ..]
        ...
        okusCounts = players & map _playerNumOkuses & zip [0 ..]
        givePointForEach :: [(PlayerIndex, Int)] -> State [Int] ()
        givePointForEach counts =
            counts & map (\(ind, num) -> givePointsTo num ind) & sequence_
        forceLookup :: PlayerIndex -> [(PlayerIndex, a)] -> a
        forceLookup playerIndex playerData =
            case lookup playerIndex playerData of
                Nothing -> error "impossible"
                Just a  -> a
    givePointForEach luminaryCounts
    givePointForEach okusCounts
    givePointForEach foolCounts
    let mostCardPlayers   = indicesOfHighest cardCounts
...
executeMove playerIndex move = case move of
    ...
    Stockpile card dir stacks desiredStackVal ->
        stockpile playerIndex card dir stacks desiredStackVal
...
$(deriveJSONAndTypeScript AesonOptions.options ''Move)
