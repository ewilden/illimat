...
byUsers = HM.foldr processGame HM.empty
    where processGame game partialMap = _gamePlayers game
            |> foldr (addEntry game) partialMap
...
createGame req = do
    ...
    atomically $ do
        ...
        let newGame = D.Game 
            ...
            newState = state
                { _sGames = HM.insert gid newGame $ _sGames state 
                }
...
startGame :: InMemory r m => D.StartGameRequest -> m (Either D.StartGameError D.StartGameResponse)
